...
// Método para forzar actualización de proyectos
const forceProjectsUpdate = () => {
  try {
...
onMounted(() => {
  if (typeof window !== 'undefined') {
    // Eventos globales
    window.addEventListener('languageChanged', forceProjectsUpdate);
    
    // Evento específico de componente
    document.addEventListener('projectsLocaleUpdated', forceProjectsUpdate);
    
    // Exponer método al objeto window para acceso directo
    if (!window.forceUpdateComponents) {
      window.forceUpdateComponents = {};
    }
    window.forceUpdateComponents.projects = forceProjectsUpdate;
  }
});
</script>
